...
describe CategoriesController do
  describe "GET index" do
    ...
    let!(:chiquita) {Fabricate(:picture, title: "Chiquita", categories: [bananas], represents_category: bananas, represents_user: bananas.user)}
    let!(:mcintosh) {Fabricate(:picture, title: "McIntosh", categories: [apples], represents_category: apples, represents_user: apples.user)}
  ...
  describe "tests where user is already signed in" do
    ...
    describe "POST #which_category_to_delete" do
      ...
      it "should redirect to the User Show page if there are one or more pictures" do
        category1 = Fabricate(:category, user: user)
        category2 = Fabricate(:category, user: user)
        photo = Fabricate(:picture, title: "Bing", categories: [category2], represents_category: category2, represents_user: user)
        post :which_category_to_delete, user_id: user.id, id: category1.id
